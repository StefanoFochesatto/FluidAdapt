...
outfile = VTKFile("bubbleshear.pvd")
T = 6.0
...
def run_simulation(mesh, t_start, t_end, c0, outfile):
    ...
    while float(t) < t_end + 0.5 * float(dt):
        ...
        lvs.solve()
        c.rename("concentation")
        outfile.write(c, t = t, adaptive = True)
...
outfile = VTKFile("bubbleshearCoarse.pvd")

mesh_coarse = UnitSquareMesh(32, 32)
c0_coarse = get_initial_condition(mesh_coarse)
c_coarse_final = run_simulation(mesh_coarse, 0.0, simulation_end_time, c0_coarse, outfile)

outfile = VTKFile("bubbleshearFine.pvd")
mesh_fine = UnitSquareMesh(128, 128)
c0_fine = get_initial_condition(mesh_fine)
c_fine_final = run_simulation(mesh_fine, 0.0, simulation_end_time, c0_fine, outfile)
...
outfile = VTKFile("bubbleshearClassical.pvd")
...
for i in range(num_adaptations):
    ...
    c = run_simulation(mesh, t0, t1, c, outfile)
...
outfile = VTKFile("bubbleshearMetricAdvection.pvd")
